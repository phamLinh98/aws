...
var handler = async (event) => {
  ...
  try {
    const resizeAvatarResponse = await fetch(resizeAvatarUrl, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json"
      }
    });
    if (!resizeAvatarResponse.ok) {
      throw new Error(`Resize Avatar API error: ${resizeAvatarResponse.statusText}`);
    }
    const resizeAvatarData = await resizeAvatarResponse.json();
    ...
    if (!updateStatusResponse.ok) {
      throw new Error(`Update Status API error: ${updateStatusResponse.statusText}`);
    }
    const updateStatusData = await updateStatusResponse.json();
    return {
      statusCode: 200,
      body: JSON.stringify({
        resizeAvatarResponse: resizeAvatarData,
        updateStatusResponse: updateStatusData
      })
    };
  } catch (error) {
    return {
